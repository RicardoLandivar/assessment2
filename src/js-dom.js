...
auth.addEventListener('click', function() {
    if (isLoggedIn) {
        isLoggedIn = false;
        loginStatus.textContent = 'You are logged out';
...
document.getElementbyId("alert-message").addEventListener('submit', function (event)
{
    event.preventDefault();
    const sendAlert = document.getElementById("send-alert").value;
    alert(sendAlert);
});
...
function changeColor(color) {
    const elements = document.querySelectorAll('.change-colors');
...
document.getElementById("factorial-calculator").addEventListener('click', function ()
{
    const numberInput = parseInt(document.getElementById("factorial-input").value);
...
const recommendWord = document.getElementById("recommend-word");
const feedback = document.getElementsByClassName("form-feedback");

recommendWord.addEventListener('submit', function (event) {
    event.preventDefault();
